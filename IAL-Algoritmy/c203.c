...
*/
	if(q == NULL) {	//kontrola existencie fronty
		queueError(QERR_INIT);
		return;
	}
	//nastavenie kazdeho prvku fronty na '*'
	for(int i = 0; i < QUEUE_SIZE; i++) {
		q->arr[i] = '*';
	}
	//vynulovanie indexu zaciatku a prveho volneho miesta fronty
	q->f_index = 0;
	q->b_index = 0;
...
*/
//riesenie z prezentacie
	//return (((q->f_index -1) == (q->b_index) || ((q->f_index == 0) && (q->b_index == QUEUE_SIZE - 1)) ? 1 : 0;
	
	//moje riesenie
	return (((q->f_index -1) == (q->b_index)) || ((q->f_index) == (nextIndex(q->b_index)))) ? 1 : 0;
...
*/
	if(q == NULL || queueEmpty(q) != 0) {
		queueError(QERR_FRONT);
		return;
	} else if (queueEmpty(q) != 0) {
		return; //nekorektne volanie funkcie
	}
	*c = q->arr[q->f_index];
...
*/
	if(q == NULL || queueEmpty(q) != 0) { //ukonci pokial je fronta prazdna/ neexistuje
...
*/
	if(q == NULL || queueFull(q) != 0) { //ukonci pokial je fronta plna/ neexistuje
		queueError(QERR_UP);
		return;
	}
	q->arr[q->b_index] = c;
