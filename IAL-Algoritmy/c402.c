...
**/
	...
	if((*RootPtr) == NULL) { //strom nema root prvok
		tBTNodePtr tmp = malloc(sizeof(struct tBTNode)); 
		if(tmp != NULL) {
			tmp->Cont = Content;
			tmp->LPtr = NULL;	
			tmp->RPtr = NULL;	
	...
	else { //toto bude fungovat iba na prve prvky, este to nejako prerobit na dalsie
		tBTNodePtr insPtr = (*RootPtr);
		tBTNodePtr insPtrLast = NULL;
		bool left = FALSE, right = FALSE;
		//tBTNodePtr insPtrLeft = NULL;
		//tBTNodePtr insPtrRight = NULL;
		while((insPtr != NULL)) { //mozno to este prerobit na do while, nech sa to na prve 2 prvky spravi
			if(insPtr->Cont > Content) { //novy je mensi, ide sa dolava
				insPtrLast = insPtr;
				insPtr = insPtr->LPtr;
				left = TRUE;
				right = FALSE;
			} else if(insPtr->Cont < Content) { //novy je vacsi, ide sa doprava
				insPtrLast = insPtr;
				insPtr = insPtr->RPtr;
				left = FALSE;
				right = TRUE;
			...
			} else { //item tam uz je, nic nerobi
				return;
			}
		}
		//insPtr == null, dostali sme sa nakoniec kde treba vlozit novy prvok
		tBTNodePtr tmp = malloc(sizeof(struct tBTNode)); 
			if(tmp != NULL) {
				tmp->Cont = Content;
				tmp->LPtr = NULL;	
...
**/
	tStackP *tmp = malloc(sizeof(tStackP));
	//mozno tStackP tmp; a potom sinitp poslat &tmp
	if(tmp != NULL) {
		SInitP(tmp);
		Leftmost_Preorder(RootPtr, tmp);
		while (!SEmptyP(tmp)) {
			RootPtr = STopPopP(tmp);
			Leftmost_Preorder(RootPtr->RPtr, tmp);
		}
	}
	
	free(tmp); //alebo to este prerobit na staticku strukturu
...
**/
	...
	if(tmp != NULL) {
		SInitP(tmp);
		Leftmost_Inorder(RootPtr, tmp);
		while (!SEmptyP(tmp)) {
			RootPtr = STopPopP(tmp);
			BTWorkOut(RootPtr);
			Leftmost_Inorder(RootPtr->RPtr, tmp);
		}
	}
...
**/
	...
	if(StackP != NULL && StackB != NULL) {
		...
		while (!SEmptyP(StackP)) {
			RootPtr = STopPopP(StackP); //ked tak toto prerobit iba na top
			SPushP(StackP, RootPtr); //a potom mozem dat toto prec
			zleva = STopPopB(StackB);
			if(zleva) { //prichadza z lava, pojde doprava
				//SPushB(&(*StackB), FALSE);
				SPushB(StackB, FALSE);
				Leftmost_Postorder(RootPtr->RPtr, StackP, StackB);
			} else {
				BTWorkOut(RootPtr);
				STopPopP(StackP); //a toto prerobit iba na stack pop
			}
		}
	}
		...
		do {
			if ((*RootPtr) == NULL) {
				...
				if((*RootPtr)->RPtr != NULL) {
					SPushP(tmp, (*RootPtr)->RPtr);
				} 
				tBTNodePtr delPtr = (*RootPtr);
				(*RootPtr) = (*RootPtr)->LPtr;
				free(delPtr);
			}
		} while(((*RootPtr) != NULL) || (!SEmptyP(tmp)));
	}
